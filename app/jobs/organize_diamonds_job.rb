  ...
  def perform(*args)
    ...
    #         #-----End_Create data table create for Table model-------------
	   ...
	   # elsif @latest_date != @oldest_date
          ...
          if uniq_date != []
          # else
          @weight03_diamond_group_all = Diamond.weight03.where(:shape => "BR").where(:date=> @last_updated_date_table..@latest_date)
          @weight04_diamond_group_all = Diamond.weight04.where(:shape => "BR").where(:date=> @last_updated_date_table..@latest_date)
          @weight05_diamond_group_all = Diamond.weight05.where(:shape => "BR").where(:date=> @last_updated_date_table..@latest_date)
          @weight06_diamond_group_all = Diamond.weight06.where(:shape => "BR").where(:date=> @last_updated_date_table..@latest_date)
          @weight07_diamond_group_all = Diamond.weight07.where(:shape => "BR").where(:date=> @last_updated_date_table..@latest_date)
          @weight08_diamond_group_all = Diamond.weight08.where(:shape => "BR").where(:date=> @last_updated_date_table..@latest_date)
          @weight09_diamond_group_all = Diamond.weight09.where(:shape => "BR").where(:date=> @last_updated_date_table..@latest_date)
          @weight10_diamond_group_all = Diamond.weight10.where(:shape => "BR").where(:date=> @last_updated_date_table..@latest_date)
          @weight12_diamond_group_all = Diamond.weight12.where(:shape => "BR").where(:date=> @last_updated_date_table..@latest_date)
          @weight15_diamond_group_all = Diamond.weight15.where(:shape => "BR").where(:date=> @last_updated_date_table..@latest_date)
          @weight18_diamond_group_all = Diamond.weight18.where(:shape => "BR").where(:date=> @last_updated_date_table..@latest_date)
          @weight20_diamond_group_all = Diamond.weight20.where(:shape => "BR").where(:date=> @last_updated_date_table..@latest_date)
          @weight30_diamond_group_all = Diamond.weight30.where(:shape => "BR").where(:date=> @last_updated_date_table..@latest_date)
          @weight40_diamond_group_all = Diamond.weight40.where(:shape => "BR").where(:date=> @last_updated_date_table..@latest_date)
